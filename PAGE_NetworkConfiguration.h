...
void send_network_configuration_html()
{
  ...
  if (server.args() > 0 )  // Save Settings
  {
    ...
    for ( uint8_t i = 0; i < server.args(); i++ ) {
      if (server.argName(i) == "ssid") stringConfig.ssid =   urldecode(server.arg(i));
      if (server.argName(i) == "password") stringConfig.password =    urldecode(server.arg(i)); 
...
void send_network_configuration_values_html()
{
  ...
  values += "ssid|" + (String) stringConfig.ssid + "|input\n";
  values += "password|" +  (String) stringConfig.password + "|input\n";
